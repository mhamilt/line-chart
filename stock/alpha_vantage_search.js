...
function alphaVantageQuery(symbol)
{
    ...
    // result[0] = ttmResult.RevenueTTM
    // result[1:5] = // result.annualReports[0:4].costOfRevenue
    ...
    financialsRequest.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let i = 1; i < 6; i++) {
                result[i] = response.annualReports[i].costOfRevenue;
    ...
    ttmRequest.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            result[0] = response.RevenueTTM;
            document.getElementById(`ttm`).innerText = response.RevenueTTM;            
